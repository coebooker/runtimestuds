...
namespace CRS
{
    public class courseDatabase
    {
        ...
        public void addNewCourse(string newCode, string newName, string newInst, string newCredit, int newSeats, List<string> newTimeBlocks, string filepath)
        {
            int numTimeBlocks = newTimeBlocks.Count();
            course newCourse = new course(newCode, newName, newInst, newCredit, newSeats, numTimeBlocks, newTimeBlocks);
            crsLst.Add(newCourse);

            //StreamWriter writer = new StreamWriter(filepath, true);
            //writer.WriteLine("\n");
            //string outputLine = newCode.PadRight(11) + newName.PadRight(16) + newInst.PadRight(11) + newCredit.PadRight(5) + newSeats.ToString().PadRight(4) + numTimeBlocks.ToString();
            //foreach (string timeBlock in newTimeBlocks)
            //{
            //    outputLine += " "+ timeBlock;
            //}
